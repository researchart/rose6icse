...
for i = 1:length(PA)
    PA(i) = PA(i) & PAhull;
    if get(PA(i), 'RCheb') >= Opt.tol_minR, keep(end+1) = i; end;
end;
if Opt.verbose>1,
...
% dimension of state-inupt space
nxu = size(Hi{1}, 2);
...
%             % MPT approach (very slow)
%             Pm = polytope(H, K);
%             Rinter = []; Cinter = []; 
%             for j = 1:length(PA)
%                 Pinter = Pm & PA(j);
%                 if isfulldim(Pinter),
%                     Rinter(end+1) = get(Pinter, 'RCheb');
%                     Cinter(end+1) = color.Reg(j);
%                 end;
%             end;
%             stat = sortrows(Rinter' Cinter'], 1);
