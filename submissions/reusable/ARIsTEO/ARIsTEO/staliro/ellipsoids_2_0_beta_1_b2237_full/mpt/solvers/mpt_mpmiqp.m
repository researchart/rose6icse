...
if ~isfield(Matrices, 'vartype'),
    error('mpt_mpmiqp: Matrices.vartype must be given.');
end
nu = size(Matrices.G, 2);
nx = size(Matrices.E, 2);
if nu~=length(Matrices.vartype),
    error('mpt_mpmiqp: Matrices.vartype has wrong dimension.');
end
...
for ii = 1:length(alph_vars),
    ...
    if ~isempty(Matrices.alphabet{ivar}),
        ...
        error(sprintf('mpt_mpmiqp: No alphabet defined in Matrices.alphabet{%d}.', ivar));
    end
end

% objective
obj = 0.5*U'*Matrices.H*U;
if isfield(Matrices, 'F'),
    obj = obj + X'*Matrices.F*U;
end
if isfield(Matrices, 'Cf'),
    obj = obj + Matrices.Cf*U;
end
if isfield(Matrices, 'Y'),
    obj = obj + X'*Matrices.Y*X;
end
if isfield(Matrices, 'Cx'),
    obj = obj + Matrices.Cx*X;
end
...
% partitions do everlap and cost is Piecewise Quadratic, therefore we just
% collect all results together
try
    if length(yalmipsol)==1,
        sol = yalmipsol{1};
    else
        sol = mpt_mergeCS(yalmipsol);
    end
