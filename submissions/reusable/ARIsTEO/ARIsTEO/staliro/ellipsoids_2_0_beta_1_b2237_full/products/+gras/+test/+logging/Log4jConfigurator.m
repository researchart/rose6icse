...
classdef Log4jConfigurator<modgen.logging.log4j.Log4jConfigurator
    ...
    properties(Constant)
        SP_MAIN_LOG_FILE_NAME='gras.testrunner.log4j.logfile.main.name'
        SP_CUR_PROCESS_NAME='gras.testrunner.log4j.curProcessName'
        SP_LOG_DIR_WITH_SEP='gras.testrunner.log4j.logfile.dirwithsep'
        SP_LOG_FILE_EXP='gras.testrunner.log4j.logfile.ext'
        CONF_REPO_MGR_CLASS=...
            'gras.test.configuration.AdaptiveConfRepoManager';
    end
    ...
    methods (Static)
        function configure(confSource,varargin)
            ...
            %
            self=gras.test.logging.Log4jConfigurator();
