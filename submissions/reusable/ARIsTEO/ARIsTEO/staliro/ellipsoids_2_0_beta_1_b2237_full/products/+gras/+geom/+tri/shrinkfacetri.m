...
%
import gras.geom.tri.mapface2edge
if nargin<5
...
while true
    ...
    if any(isE2OrigPartVec)&&(iStep<=nMaxSteps)
        ...
        %
        f2e2NewMat=nEdges+...
            [repmat(indShiftEdgeVec,3,1)+kron([1;0;2]*nShrinkedFaces,ones(nShrinkedFaces,1)),...%edges from the first group
            3*nShrinkedFaces-nVerts+dirMat*nNewVerts+... %shift indices for vertices correspond to shift idices for edges
            [indVF13Vec indVF12Vec;indVF12Vec indVF23Vec;indVF23Vec indVF13Vec]];
        %
